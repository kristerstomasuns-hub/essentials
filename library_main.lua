...
local function patchMacLibSource(macSource)
	...
	macSource = macSource:gsub("ghostSkull%.Position = UDim2%.new%(0, 0, 1, %-45%)", "ghostSkull.Position = UDim2.new(0, -16, 0.5, 0)")
	macSource = macSource:gsub("ghostSkull%.Size = UDim2%.fromOffset%(90, 90%)", "ghostSkull.Size = UDim2.fromOffset(84, 84)")
	macSource = macSource:gsub("ghostWordmark%.Position = UDim2%.new%(0, 68, 1, %-146%)", "ghostWordmark.Position = UDim2.fromOffset(58, -49)")
	macSource = macSource:gsub("ghostWordmark%.Size = UDim2%.fromOffset%(225, 225%)", "ghostWordmark.Size = UDim2.fromOffset(205, 205)")
	macSource = macSource:gsub("ghostLuaL%.Position = UDim2%.new%(0, 68, 1, %-146%)", "ghostLuaL.Position = UDim2.fromOffset(58, -49)")
	macSource = macSource:gsub("ghostLuaL%.Size = UDim2%.fromOffset%(225, 225%)", "ghostLuaL.Size = UDim2.fromOffset(205, 205)")
	macSource = macSource:gsub("ghostLua%.Position = UDim2%.new%(0, 68, 1, %-146%)", "ghostLua.Position = UDim2.fromOffset(58, -49)")
...
local function tuneMacSidebarLayout(gui)
	...
	if ghostLogo and informationHolder then
		...
		if skull then
			...
			skull.Position = UDim2.new(0, -16, 0.5, 0)
		...
		for _, name in ipairs({ "Wordmark", "LuaL", "Lua" }) do
			...
			if logoPart then
				...
				logoPart.Position = UDim2.fromOffset(58, -49)
