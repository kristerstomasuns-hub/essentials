...
local function patchMacLibSource(macSource)
	...
	macSource = macSource:gsub("ghostSkull%.Size = UDim2%.fromOffset%(90, 90%)", "ghostSkull.Size = UDim2.fromOffset(94, 94)")
	macSource = macSource:gsub("ghostWordmark%.Position = UDim2%.new%(0, 68, 1, %-146%)", "ghostWordmark.Position = UDim2.fromOffset(58, -22)")
	macSource = macSource:gsub("ghostWordmark%.Size = UDim2%.fromOffset%(225, 225%)", "ghostWordmark.Size = UDim2.fromOffset(232, 232)")
	macSource = macSource:gsub("ghostLuaL%.Position = UDim2%.new%(0, 68, 1, %-146%)", "ghostLuaL.Position = UDim2.fromOffset(58, -22)")
	macSource = macSource:gsub("ghostLuaL%.Size = UDim2%.fromOffset%(225, 225%)", "ghostLuaL.Size = UDim2.fromOffset(232, 232)")
...
local function tuneMacSidebarLayout(gui)
	...
	if ghostLogo and informationHolder then
		...
		ghostLogo.Position = UDim2.new(0, -18, 0, -12)
		...
		if skull then
			...
			skull.Size = UDim2.fromOffset(94, 94)
		end
		for _, name in ipairs({ "Wordmark", "LuaL", "Lua" }) do
			...
			if logoPart then
				...
				logoPart.Position = UDim2.fromOffset(58, -22)
				logoPart.Size = UDim2.fromOffset(232, 232)
			end
		end
	end
