...
local function patchMacLibSource(macSource)
	...
	macSource = macSource:gsub("ghostLogo%.Position = UDim2%.new%(0, 4, 1, %-12%)", "ghostLogo.Position = UDim2.new(0, 0, 0, -7)")
	macSource = macSource:gsub("ghostLogo%.Size = UDim2%.new%(1, %-6, 0, 104%)", "ghostLogo.Size = UDim2.new(1, -12, 1, 8)")
	...
	macSource = macSource:gsub("ghostSkull%.Size = UDim2%.fromOffset%(90, 90%)", "ghostSkull.Size = UDim2.fromOffset(76, 76)")
	macSource = macSource:gsub("ghostWordmark%.Position = UDim2%.new%(0, 68, 1, %-146%)", "ghostWordmark.Position = UDim2.fromOffset(58, -54)")
	macSource = macSource:gsub("ghostWordmark%.Size = UDim2%.fromOffset%(225, 225%)", "ghostWordmark.Size = UDim2.fromOffset(194, 194)")
	macSource = macSource:gsub("ghostLuaL%.Position = UDim2%.new%(0, 68, 1, %-146%)", "ghostLuaL.Position = UDim2.fromOffset(58, -54)")
	macSource = macSource:gsub("ghostLuaL%.Size = UDim2%.fromOffset%(225, 225%)", "ghostLuaL.Size = UDim2.fromOffset(194, 194)")
	macSource = macSource:gsub("ghostLua%.Position = UDim2%.new%(0, 68, 1, %-146%)", "ghostLua.Position = UDim2.fromOffset(58, -54)")
	macSource = macSource:gsub("ghostLua%.Size = UDim2%.fromOffset%(225, 225%)", "ghostLua.Size = UDim2.fromOffset(194, 194)")
	...
	macSource = macSource:gsub("Transparency = %(i == tabSwitcher and 0%.95 or 1%)", "Transparency = (i == tabSwitcher and 0.5 or 1)")
	...
	themeAccentLineRight.BackgroundTransparency = 0.5
...
local function tuneMacSidebarLayout(gui)
	...
	if ghostLogo and informationHolder then
		...
		ghostLogo.Position = UDim2.new(0, 0, 0, -7)
		ghostLogo.Size = UDim2.new(1, -12, 1, 8)
		...
		if skull then
			...
			skull.Size = UDim2.fromOffset(76, 76)
		end
		for _, name in ipairs({ "Wordmark", "LuaL", "Lua" }) do
			...
			if logoPart then
				...
				logoPart.Position = UDim2.fromOffset(58, -54)
				logoPart.Size = UDim2.fromOffset(194, 194)
			end
		end
	end
	...
	for _, object in ipairs(gui:GetDescendants()) do
		if object:IsA("UIStroke") and object.Name == "TabSwitcherUIStroke" then
			...
			object.Transparency = (parent and parent:IsA("GuiObject") and parent.BackgroundTransparency < 1) and 0.5 or 1
		end
	end
end
...
function library.new(libraryTitle, cfgLocation)
	...
	local function isForcedThemeAccentProperty(object, property)
		...
		if property == "BackgroundColor3" and (isToggleTrack(object) or object.Name == "SliderFrame" or object.Name:match("^ThemeAccentLine")) then
			return true
		end

		if property == "ImageColor3" and (isToggleTrack(object) or object.Name == "SliderBar") then
			return true
		end

		if property == "Color" and object:IsA("UIStroke") and (object.Name == "ToggleUIStroke" or object.Name == "ColorPickerOutline" or object.Name == "TabSwitcherUIStroke") then
			return true
		end
	...
	function applyThemeAccentToObject(object)
		...
		if object:IsA("GuiObject") and object.Name:match("^ThemeAccentLine") then
			pcall(function()
				...
				object.BackgroundTransparency = object.Name == "ThemeAccentLineRight" and 0.5 or 1
